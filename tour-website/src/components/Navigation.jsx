...
export default function Navigation() {
  ...
  const socialIcons = [
    { Icon: "facebook", path: "M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z" },
    ...
    { Icon: "linkedin", path: "M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.5 2.48-2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.593-11.018-3.714v-2.155z" },
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center py-2 gap-2 text-white text-sm">
            <div className="flex items-center gap-6">
              <span className="flex items-center gap-2 hover:text-blue-200 transition-colors duration-300">
              ...
              <span className="hidden lg:block animate-pulse">Welcome to Amazing Tours</span>
            </div>
      ...
      >
            ...
            <div className="hidden lg:flex items-center gap-6">
              {menuItems.map((item, index) => (
                <a
                  key={item.label}
                  href={item.path}
                  className="relative text-sm font-semibold text-gray-700 hover:text-blue-600 transition-all duration-300 group py-2"
                  style={{ animationDelay: `${index * 50}ms` }}
                >
                  {item.label}
                  <span 
                    className="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-blue-600 to-blue-400 group-hover:w-full transition-all duration-500 ease-out"
                  ></span>
                  <span 
                    className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-blue-600 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                  ></span>
                </a>
              ))}
            </div>
        ...
        >
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-bold text-lg text-white">Amazing Tours</h2>
          ...
          {/* Menu Items */}
          <div className="flex flex-col py-6 px-4 space-y-1 overflow-y-auto h-[calc(100%-200px)]">
            {menuItems.map((item, index) => (
              <a
                key={item.label}
                href={item.path}
                onClick={() => setIsMenuOpen(false)}
                className="group block py-4 px-6 rounded-xl text-gray-700 font-semibold hover:bg-blue-600 hover:text-white transition-all duration-300 transform hover:translate-x-2 hover:shadow-lg"
                style={{ 
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white to-transparent">
            <a
              href="tel:01898760770"
              className="flex items-center justify-center gap-3 py-4 rounded-xl font-bold text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105"
