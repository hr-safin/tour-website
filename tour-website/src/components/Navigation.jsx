  ...
  return (
    <>
              ...
              {socialIcons.map((social, i) => (
                <a
                  ...
                  className="w-7 h-7 rounded-full border border-white/30 flex items-center justify-center
                    hover:bg-white hover:text-blue-900 hover:scale-110 hover:rotate-12 transition-all duration-300"
      ...
      >
          ...
          <div className="flex justify-between items-center h-20">
            ...
            >
              <div className="w-12 h-12 sm:w-14 sm:h-14 flex-shrink-0 transition-all duration-500
                ...
                <img
                  src="https://i.postimg.cc/SRpm2NJg/timelogo.png"
                  alt="Time Ex Tours & Travels"
            ...
            <div className="hidden lg:flex items-center gap-3 xl:gap-5 flex-1 justify-center px-4">
              {menuItems.map((item, index) => (
                <Link
                  ...
                  to={item.path}
                  onClick={handleMenuClick}
                  className={`relative text-[11px] xl:text-xs font-semibold transition-all duration-300
                    group py-2 whitespace-nowrap ${
            ...
            <div className="hidden lg:flex items-center flex-shrink-0">
              <a
                ...
                className="relative flex items-center gap-2 px-5 py-3 rounded-full font-semibold
                  text-white overflow-hidden group shadow-lg hover:shadow-2xl transition-all duration-300"
                style={{ backgroundColor: "#1D4ED8" }}
              >
                <span className="absolute inset-0 w-0 bg-gradient-to-r from-blue-700 to-blue-600
                  transition-all duration-500 ease-out group-hover:w-full" />
                <svg
                  className="w-4 h-4 relative z-10 group-hover:rotate-12 transition-all duration-300"
                  fill="currentColor" viewBox="0 0 20 20"
                >
                  <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" />
                </svg>
                <span className="relative z-10 text-sm">Call Now</span>
                <span className="absolute top-0 left-0 w-2 h-2 bg-white/30 rounded-full animate-ping" />
              </a>
            </div>
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-xl hover:bg-blue-50 transition-all duration-300 flex-shrink-0"
              aria-label="Toggle menu"
            >
              <div className="w-6 space-y-1.5">
                <span className={`block h-0.5 bg-blue-600 rounded-full transition-all duration-300 ${isMenuOpen ? "rotate-45 translate-y-2" : ""}`} />
                <span className={`block h-0.5 bg-blue-600 rounded-full transition-all duration-300 ${isMenuOpen ? "opacity-0" : ""}`} />
                <span className={`block h-0.5 bg-blue-600 rounded-full transition-all duration-300 ${isMenuOpen ? "-rotate-45 -translate-y-2" : ""}`} />
              </div>
            </button>
        ...
        {/* ── Mobile slide-out ── */}
        <div
          className={`lg:hidden fixed top-0 left-0 w-72 h-full bg-white shadow-2xl z-50
            transform transition-all duration-500 ease-in-out flex flex-col ${
            isMenuOpen ? "translate-x-0" : "-translate-x-full"
          }`}
        >
          {/* Header — plain white, no coloured bg */}
          <div className="flex items-center justify-between px-5 py-5 border-b border-gray-100 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-sm leading-tight" style={{ color: "#1D4ED8" }}>
                  Time Ex Tours
                </p>
            ...
            <button
              ...
              className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 transition-all duration-300 hover:rotate-90"
          ...
          <div className="flex-1 overflow-y-auto py-3 px-3 space-y-0.5">
            {menuItems.map((item, index) => (
              <Link
                ...
                to={item.path}
                onClick={handleMenuClick}
                className={`group flex items-center justify-between py-3.5 px-4 rounded-xl
                ...
                style={{
                  background: location.pathname === item.path ? "#1D4ED8" : "transparent",
                  animation: isMenuOpen ? `slideIn 0.3s ease-out ${index * 0.04}s both` : "none",
                }}
                onMouseEnter={e => {
                  if (location.pathname !== item.path) e.currentTarget.style.background = "#1D4ED8";
                }}
                onMouseLeave={e => {
              ...
              >
                ...
                <svg
                  className="w-4 h-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-1
